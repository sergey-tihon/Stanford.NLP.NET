...
module Common =
    ...
    let getFullPath path = FileInfo(path).FullName
...
module CoreNLP =
    let jarRoot = nlpStanford </> "stanford-corenlp-4.0.0/models/"
...
module Segmenter =
    let data path = getFullPath <| ( nlpStanford </> "stanford-segmenter-4.0.0/data/" </> path)

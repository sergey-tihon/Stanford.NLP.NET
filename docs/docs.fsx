...
let processScripts siteRoot force =
    Helpers.processDirectory force source output [".fsx"]
      (fun source outdir ->
          let name = Path.GetFileNameWithoutExtension(source)
          printfn "Processing script file: %s.fsx" name
          use tmp = TempFile.New()
          let attrs = System.Collections.Generic.Dictionary<_, _>()
          let heading = ref ""
          Literate.ProcessScriptFile(source, output=tmp.Name, generateAnchors=true, customizeDocument = fun _ doc ->
              let kvps, pars = extractAttributes name doc.MarkdownDocument doc.Paragraphs
              for k, v in kvps do attrs.Add(k, v)
              let htext, pars = extractHeading name pars
              heading.Value <- htext

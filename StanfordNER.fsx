...
let classifiedLabels = classifier.classify(s1)
let cliqueTree = classifier.getCliqueTree(classifiedLabels)

for i in [0..cliqueTree.length()-1] do
    let wi = classifiedLabels.get(i)
    let rec pp (iter:java.util.Iterator) =
        let label = iter.next()
        let index = classifier.classIndex.indexOf(label)
        let prob = cliqueTree.prob(i, index)
        printfn "\t%A(%f)" label prob
        if iter.hasNext() then pp iter
    pp <| classifier.classIndex.iterator()

    let tag = wi.get(CoreAnnotations.AnswerAnnotation().getClass())
    printfn "Class : %s" tag
...
-----------------------------
    ...
    namespace ner
    {
        class Program
        {
            ...
            {
                // Path to the folder with classifies models
                var jarRoot = @"c:models\stanford-ner-2015-01-30";

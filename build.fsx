...
type coreNLPDir = root.data.``paket-files``.``nlp.stanford.edu``.``stanford-corenlp-full-2018-02-27``

Target "CompilerCoreNLP" (fun _ ->
    let ikvmDir  = @"bin\Stanford.NLP.CoreNLP\lib"
    CreateDir ikvmDir

    coreNLPDir.``stanford-corenlp-3.9.1-models.jar``
    |> restoreFolderFromFile (Path.Combine(coreNLPDir.Path, "models"))

    let jodaTime = IKVMcTask(coreNLPDir.``joda-time.jar``, version="2.9.4")
    [IKVMcTask(coreNLPDir.``stanford-corenlp-3.9.1.jar``, version=release.AssemblyVersion,
...
type nerDir = root.data.``paket-files``.``nlp.stanford.edu``.``stanford-ner-2018-02-27``
...
type parserDir = root.data.``paket-files``.``nlp.stanford.edu``.``stanford-parser-full-2018-02-27``

Target "CompilerParser" (fun _ ->
    let ikvmDir  = @"bin\Stanford.NLP.Parser\lib"
    CreateDir ikvmDir

    restoreFolderFromFile (parserDir.Path + "models") parserDir.``stanford-parser-3.9.1-models.jar``
...
type posDir = root.data.``paket-files``.``nlp.stanford.edu``.``stanford-postagger-full-2018-02-27``

Target "CompilerPOS" (fun _ ->
    let ikvmDir  = @"bin\Stanford.NLP.POSTagger\lib"
    CreateDir ikvmDir

    [IKVMcTask(posDir.``stanford-postagger-3.9.1.jar``, version=release.AssemblyVersion,
        Dependencies = [])]
    |> IKVMCompile ikvmDir keyFile
)
...
type segmenterDir = root.data.``paket-files``.``nlp.stanford.edu``.``stanford-segmenter-2018-02-27``

Target "CompilerSegmenter" (fun _ ->
    let ikvmDir  = @"bin\Stanford.NLP.Segmenter\lib"
    CreateDir ikvmDir

    [IKVMcTask(segmenterDir.``stanford-segmenter-3.9.1.jar``, version=release.AssemblyVersion,
        Dependencies=[])]
    |> IKVMCompile ikvmDir keyFile
)
